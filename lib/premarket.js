...
const INDICES=[["SPY","SPX","S&P 500"],["QQQ","NDX","Nasdaq 100"],["DIA","DJI","Dow Jones"],["IWM","RUT","Russell 2000"]];
...
export default async function handler(req, res){
  ...
  try{
    const [etfs, indices, stocks] = await Promise.all([
      fhSet(ETFS), fhSet(INDICES), fhSet(STOCKS.map(s=>[s,s,s])),
    ]);
    const sv=stocks.filter(x=>x.p!=null);
    const gainers=[...sv].sort((a,b)=>b.chg-a.chg).slice(0,8);
    const losers =[...sv].sort((a,b)=>a.chg-b.chg).slice(0,8);
    const mostActive=[...sv].sort((a,b)=>Math.abs(b.chg)-Math.abs(a.chg)).slice(0,12);
