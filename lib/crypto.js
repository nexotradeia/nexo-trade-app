...
export default async function handler(req, res) {
  ...
  try {
    // CoinGecko free tier limita seguido (429), sobre todo con sparkline. Reintentos + versión ligera de respaldo.
    const _sleep = ms => new Promise(r => setTimeout(r, ms));
    const cgMarkets = async () => {
      const urls = [
        ...
        "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=30&page=1&price_change_percentage=24h,7d", // sin sparkline (más ligero)
      ];
      for (let attempt = 0; attempt < 4; attempt++) {
        ...
        await _sleep(600 * (attempt + 1));
      }
      return null;
    };

    const [globR, fngR, markets] = await Promise.all([
      fetch("https://api.coingecko.com/api/v3/global", { signal: AbortSignal.timeout(8000), headers: { accept: "application/json" } }).then(r => ({ status: "fulfilled", value: r })).catch(e => ({ status: "rejected", reason: e })),
      fetch("https://api.alternative.me/fng/?limit=1", { signal: AbortSignal.timeout(8000) }).then(r => ({ status: "fulfilled", value: r })).catch(e => ({ status: "rejected", reason: e })),
      cgMarkets(),
    ]);

    if (!markets) throw new Error("CoinGecko markets unavailable");
