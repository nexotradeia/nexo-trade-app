...
export default async function finder(req, res){
  try{
    const results = await Promise.allSettled(UNIVERSE.map(async sym=>{
      const t=await fetchTech(sym); if(!t) return null;
      try{ return buildRow(sym,t); }catch(e){ return null; }
    }));
    ...
    // resultado parcial: caché corta para que siga reintentando y calentando el resto
    res.setHeader("Cache-Control","s-maxage=90, stale-while-revalidate=600");
    return res.status(200).json({ rows, n:rows.length, warming:rows.length<18, asOf:new Date().toISOString() });
  }catch(e){
