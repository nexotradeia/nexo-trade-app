...
// Velas diarias REALES vía Twelve Data en UNA sola llamada en lote (comma-separated).
async function tdBatch(syms){ const out={};
  try{
    const u=`https://api.twelvedata.com/time_series?symbol=${syms.join(",")}&interval=1day&outputsize=90&apikey=${TD_KEY}`;
    const r=await fetch(u,{signal:AbortSignal.timeout(9500)}); if(!r.ok) return out;
    const j=await r.json();
    if(j && Array.isArray(j.values)){ const one=parseTD(j); if(one) out[syms[0]]=one; return out; } // respuesta de 1 símbolo
    for(const s of syms){ if(j[s]){ const one=parseTD(j[s]); if(one) out[s]=one; } }
  }catch(e){}
  return out;
}

async function getCandles(sym){ return (await stooqCandles(sym)) || (await yahooCandles(sym)); }
...
export default async function finder(req, res){
  try{
    const cmap = await tdBatch(UNIVERSE);  // velas reales (1 sola llamada Twelve Data)
    const results = await Promise.allSettled(UNIVERSE.map(async sym=>{
      let c = cmap[sym] || await getCandles(sym);   // respaldo Stooq/Yahoo si TD no trajo este símbolo
      if(!c) return null;
    ...
    return res.status(200).json({ rows:[], n:0, error:"no data" });
  }catch(e){
    res.setHeader("Cache-Control","no-store");
