...
export default async function handler(req, res){
  ...
  if(req.query.debug){
    ...
    const probes=[
      ["finnhub_spy",`https://finnhub.io/api/v1/quote?symbol=SPY&token=${FK}`],
      ["frankfurter",`https://api.frankfurter.app/latest?from=USD&to=EUR,GBP,JPY`],
      ["treasury",`https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v2/accounting/od/avg_interest_rates?page%5Bsize%5D=1`],
      ["erapi",`https://open.er-api.com/v6/latest/USD`],
      ["twelvedata",`https://api.twelvedata.com/price?symbol=AAPL`],
    ];
    const out={};
    await Promise.all(probes.map(async([n,u])=>{try{const r=await fetch(u,{headers:{"User-Agent":"Mozilla/5.0","Accept":"application/json"},signal:AbortSignal.timeout(8000)});const t=await r.text();out[n]={status:r.status,sample:t.slice(0,140)};}catch(e){out[n]={err:e.message};}}));
