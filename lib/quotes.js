...
async function fhSet(defs){
  ...
  for(let i=0;i<defs.length;i+=CHUNK){
    ...
    const out=await Promise.allSettled(part.map(async([sym,s,n])=>{const q=await fh(sym);if(!q)return null;return {s,n,p:q.p,chg:q.chg,regChg:q.chg,vol:null,volFmt:"—"};}));
