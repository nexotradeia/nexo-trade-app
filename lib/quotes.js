...
export default async function handler(req, res){
  ...
  if(req.query.debug){
    const sym=String(req.query.debug);
    try{
      const r=await fetch(`https://stooq.com/q/d/l/?s=${encodeURIComponent(sym)}&i=d`,{headers:UA,signal:AbortSignal.timeout(8000)});
      const t=await r.text();
      res.setHeader("Cache-Control","no-store");
      return res.status(200).json({ sym, status:r.status, len:t.length, sample:t.slice(0,300) });
    }catch(e){ res.setHeader("Cache-Control","no-store"); return res.status(200).json({ sym, fetchError:e.message }); }
  }
  try{
    if(set==="overview"){
