...
const TD_COMMS=[["XAU/USD","GOLD","Gold"],["XAG/USD","SILVER","Silver"],["WTI/USD","WTI","Crude Oil (WTI)"],["XPT/USD","PLATINUM","Platinum"],["XPD/USD","PALLADIUM","Palladium"],["XCU/USD","COPPER","Copper"]];
async function tdCommodities(){
  if(!TD_KEY) return [];
  const r=await fetch(`https://api.twelvedata.com/quote?symbol=${encodeURIComponent(TD_COMMS.map(c=>c[0]).join(","))}&apikey=${TD_KEY}`,{signal:AbortSignal.timeout(9000)});
  if(!r.ok) throw new Error("td "+r.status);
  const j=await r.json();
  return TD_COMMS.map(([sym,s,n])=>{const q=j[sym]!==undefined?j[sym]:(TD_COMMS.length===1?j:null);if(!q||q.close==null||q.status==="error")return null;const p=parseFloat(q.close);let chg=q.percent_change!=null?+(+q.percent_change).toFixed(2):null;if(chg==null&&q.previous_close)chg=+(((p-parseFloat(q.previous_close))/parseFloat(q.previous_close))*100).toFixed(2);return {s,n,p,chg:chg??0,regChg:chg??0,vol:null,volFmt:"—"};}).filter(Boolean);
}
