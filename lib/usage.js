...
export default async function usage(req, res){
  ...
  try{
    ...
    const today      = num(j.daily_usage) ?? null;
    const todayLimit = num(j.plan_daily_limit) ?? null;
    const perMin      = num(j.current_usage) ?? null;
    const perMinLimit = num(j.plan_limit) ?? null;
    return res.status(200).json({ ok:true, today, todayLimit, perMin, perMinLimit, plan:j.plan_category||null, ts:j.timestamp||null });
  }catch(e){
