...
async function nextEarnings(symbol){
  try{
    const today=new Date(); const to=new Date(today.getTime()+120*864e5);
    const f=d=>d.toISOString().slice(0,10);
    const r=await fetch(`https://finnhub.io/api/v1/calendar/earnings?from=${f(today)}&to=${f(to)}&symbol=${symbol}&token=${FK}`,{signal:AbortSignal.timeout(7000)});
...
export default async function technical(req, res){
  ...
  try{
    ...
    let metric=null, nextEarn=null; if(!isCrypto){ [metric, nextEarn]=await Promise.all([finnhubMetric(symbol), nextEarnings(symbol)]); }
    ...
    return res.status(200).json({symbol, ...out, metric, nextEarn, volToday, avgVol10, asOf:new Date().toISOString()});
