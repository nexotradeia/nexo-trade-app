...
export default async function handler(req, res){
  ...
  try{
    ...
    // Mostrar hasta las 30 acciones del universo (el front limita a 5 para usuarios gratis)
    const mostActive = [...withMove].filter(x=>x.vol!=null).sort((a,b)=>(b.vol||0)-(a.vol||0)).slice(0,30);
    const gainers = [...withMove].sort((a,b)=>b.chg-a.chg).slice(0,30);
    const losers  = [...withMove].sort((a,b)=>a.chg-b.chg).slice(0,30);
