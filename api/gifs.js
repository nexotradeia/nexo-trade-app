...
export default async function handler(req, res) {
  ...
  try {
    const tenorUrl = q
      ? `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent(q)}&key=${TENOR_KEY}&limit=24&media_filter=tinygif&contentfilter=medium&locale=es`
      : `https://tenor.googleapis.com/v2/search?q=trading+stocks+money&key=${TENOR_KEY}&limit=24&media_filter=tinygif&contentfilter=medium`;
